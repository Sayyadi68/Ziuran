...
// UserLocations Component
const UserLocations = ({ addresses }) => (
  <div className="mt-8">
    <h2 className="text-lg font-bold text-white mb-4 font-[Byekan]">آدرس های من</h2>
    {addresses.map((addr) => (
      <div key={addr.id} className="mb-4 p-4 bg-gray-800 rounded-lg shadow text-white font-[Byekan] flex items-start space-x-4 rtl:space-x-reverse">
        <img src={addr.mapImage} alt="نقشه" className="w-40 h-20 object-cover rounded" />
        <div className="flex-1">
          <div className="text-yellow-500 text-sm mb-1">آدرس {addr.name}</div>
        ...
        <div className="flex space-x-2 rtl:space-x-reverse">
          <button className="text-white hover:text-gray-300"><FaEdit /></button>
          <button className="text-white hover:text-gray-300"><FaTrash /></button>
        </div>
      </div>
    ))}
  </div>
        ...
        <div className="">
          ...
          {isActive('/account/userdetail') && (
            <div className="max-w-3xl mx-auto bg-gray-900 p-6 rounded-lg text-white font-[Byekan]">
              ...
              <UserLocations addresses={userAddresses} />
