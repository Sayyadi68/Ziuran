...
import { useDispatch, useSelector } from 'react-redux'  // اضافه کردن این خط
import products from '../pages/productsInfo'
import { addToCart, incrementQuantity, decrementQuantity, removeFromCart } from '../redux/cartSlice'  // اضافه کردن اکشن‌های ریداکس
...
const SingleProduct = () => {
  ...
  // بررسی موجود بودن کالا در سبد خرید
  const existingItem = cart.find((item) => item.id === productId)

  const handleAddToCart = () => {
    dispatch(addToCart(currentProduct)) // ارسال محصول به ریداکس
  }

  const handleIncrement = () => {
    dispatch(incrementQuantity(productId)) // افزایش تعداد کالا
  }

  const handleDecrement = () => {
    dispatch(decrementQuantity(productId)) // کاهش تعداد کالا
  }

  const handleRemoveFromCart = () => {
    dispatch(removeFromCart(productId)) // حذف محصول از سبد خرید
  }
    ...
    <div className='grid grid-cols-3 gap-4 items-start p-6 gap-x-4'>
      ...
      <div className='flex flex-col items-center'>
        <button
          ...
          className={`flex items-center text-lg px-4 py-2 rounded-md transition-colors duration-300 ${isFavorite ? 'bg-[#FF4D6D]' : 'text-gray-500'
            }`}
        >
        ...
        {/* نمایش دکمه‌ها برای مدیریت تعداد کالا */}
        {!existingItem ? (
          <button
